...
<head>
    <meta charset="UTF-8">
    <title>Jokerpedia: Register</title>
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["username"]))){
        ...
        $username_err = "Please enter a valid username format, Alphabets and Numbers ONLY! ";
    } else{
    ...
    if(empty($password_err) && empty($confirm_password_err) && empty($username_err) && empty($email_err)){
        ...
        if($query = $mysqli->prepare($sql)){
            // Bind variables to the prepared statement as parameters
            $query->bind_param("sss", $param_username, $param_password, $param_email);
...
?>
    ...
    <div class="wrapper">
        <h2>Jokerpedia: Register</h2>
        <p>Please fill this form to create an account.</p>
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            <div class="wrapper">
                <label>Username</label>
                <input type="text" name="username" placeholder="Abc123" class="form-control <?php echo (!empty($username_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $username; ?>">
                <span class="invalid-feedback"><?php echo $username_err; ?></span>
            </div> 			
            <div class="wrapper">
                <label>Email</label>
                <input type="text" name="email" placeholder="abc123@abc123.com" class="form-control <?php echo (!empty($email_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $email; ?>">
